...
class Expert(nn.Module):
    def __init__(self,
        ...
        for _ in range(num_SRIRs):
            body += [sp.SRIR(self.bank, num_SResidualBlocks)]
        ...
        for i in range(num_SRIRs):
            for j in range(num_SResidualBlocks):
                self.body[i].body[j].body[0].coefficients.data = coef_inits[(
                    i*num_SResidualBlocks)+(2*j)]
                self.body[i].body[j].body[2].coefficients.data = coef_inits[(
